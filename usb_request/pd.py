...
class pcap_usb_pkt():
    ...
    def __init__(self, req, ts, is_submit):
        ...
        elif req['type'] in ('ISO IN', 'ISO OUT'):
            self.set_transfertype(0) # ISO
            if req['type'] == 'ISO IN':
                self.set_addr_ep(req['addr'], 0x80 | req['ep'])
        elif req['type'] in ('BULK IN'):
            self.set_addr_ep(req['addr'], 0x80 | req['ep'])
...
class Decoder(srd.Decoder):
    ...
    annotations = (
        ...
        ('request-iso-read', 'ISO: Device-to-host'),
        ('request-iso-write', 'ISO: Host-to-device'),
    ...
    def handle_transfer(self):
        request_started = 0
        # ISO transfers don't have handshake, so request_end is True when handshake is None
        # and we have data (ISO) or when handshake is ACK/STALL/timeout (other types)
        request_end = (self.handshake is None and self.transaction_state == 'DATA RECEIVED') or \
        ...
        if request_end:
            ...
            if self.handshake is not None:
                ...
                # ISO transfers don't have handshake
                request['handshake'] = 'ISO'

        # ISO transfer - no handshake, completes immediately after data packet
        # Each ISO transaction is independent and completes immediately
        if request['type'] in (None, 'ISO IN') and self.transaction_type == 'IN' and self.handshake is None:
            request['type'] = 'ISO IN'
        ...
        elif request['type'] in (None, 'ISO OUT') and self.transaction_type == 'OUT' and self.handshake is None:
            request['type'] = 'ISO OUT'
            request['data'] += self.transaction_data
            self.handle_request(request_started, request_end)

        # BULK or INTERRUPT transfer
        elif request['type'] in (None, 'BULK IN') and self.transaction_type == 'IN' and self.handshake is not None:
            request['type'] = 'BULK IN'
            if len(request['data']) == 0 and len(self.transaction_data) > 0:
                request['ss_data'] = self.ss_transaction
            request['data'] += self.transaction_data
            self.handle_request(request_started, request_end)
        elif request['type'] in (None, 'BULK OUT') and self.transaction_type == 'OUT' and self.handshake is not None:
    ...
    def handle_request(self, request_start, request_end):
        ...
        if request_end == 1:
            ...
            elif request['type'] == 'ISO IN':
                self.putr(ss_data if ss_data else ss, es, [4, ['ISO in: %s' % summary]])
            elif request['type'] == 'ISO OUT':
                self.putr(ss, es, [5, ['ISO out: %s' % summary]])

            # Output Python data for higher-level decoders
            request_data = {
                'type': request['type'],
                'addr': request['addr'],
                'ep': request['ep'],
                'setup_data': bytes(request['setup_data']) if request['setup_data'] else None,
                'data': bytes(request['data']) if request['data'] else b'',
    ...
    def decode(self, ss, es, data):
        ...
        if pcategory == 'TOKEN':
            if pname == 'SOF':
                # If we have a pending DATA RECEIVED state without handshake,
                # this is likely an ISO transfer completing
                # es_transaction should already be set to the data packet end time
                if self.transaction_state == 'DATA RECEIVED' and self.handshake is None:
                    # es_transaction is already set to data packet end, don't change it
            ...
            if self.transaction_state == 'TOKEN RECEIVED':
                ...
                if ss > transaction_timeout:
                    ...
                    self.handle_transfer()
                    self.transaction_state = 'IDLE'
            elif self.transaction_state == 'DATA RECEIVED' and self.handshake is None:
                # Received a new TOKEN while DATA RECEIVED with no handshake
                # This means the previous transfer was ISO (no handshake packet)
                # es_transaction is already set to data packet end, don't change it
                self.handle_transfer()
                self.transaction_state = 'IDLE'

            if self.transaction_state != 'IDLE':
                self.putr(ss, es, [6, ['ERR: received %s token in state %s' %
            ...
            self.es_transaction = es  # Update end time to data packet end
            self.transaction_state = 'DATA RECEIVED'
            # For ISO transfers, there's no handshake packet, so we need to
            # handle the transfer immediately. We'll check if next packet is
            # a TOKEN (not HANDSHAKE) to confirm it's ISO.
            # For now, we set handshake to None and let handle_transfer decide.
            # If a handshake packet arrives later, it will override this.
            self.handshake = None
            # Don't call handle_transfer here yet - wait to see if handshake arrives
            ...
            if self.transaction_state == 'DATA RECEIVED' and self.handshake is None:
                ...
                self.putr(ss, es, [6, ['ERR: received unhandled %s token in state %s' %
                    (pname, self.transaction_state)]])
            return
